...
const serverUrl = window.location.host.indexOf('localhost') >= 0 ? 'http://localhost:49266/' : '/';
...
export class EsService {
    ...
    public async search<T>(index: string, searchBody: EsSearch): Promise<{ total: number; items: T[] }> {
        const rawResults = await this.call<EsResult<T>>('post', `api/es/proxy?/${index}/_search`, searchBody);
        let items: T[] = [],
            total = 0;
