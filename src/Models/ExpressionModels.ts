...
export const exprTypes = new Map<
  string,
  { ctor: new () => IExpression; condition?: (expr: any) => boolean }[]
>();
export const exprTypeNames = new Map<
  Function,
...
export abstract class VisitorBase<T> {
  public visit(expr?: IExpression): T {
    ...
    throw `I don't know how to visit this... "${
...
export class Serializer {
  static toJson(expr: IExpression) {
    ...
    if (expr instanceof Array) {
      ...
      typeof expr === "object" &&
      expr.constructor &&
      exprTypeNames.has(expr.constructor)
    ) {
  ...
  static fromJson(exprJson: any) {
    ...
    if (exprJson instanceof Array) {
      ...
      const ExprTypes = exprTypes.get(exprJson.$type),
        BestType =
          ExprTypes!.find((t) => !!t.condition && t.condition(exprJson)) ||
          ExprTypes!.find((t) => !t.condition);
