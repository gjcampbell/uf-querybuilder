...
interface ITreeOptions {
  nodeRenderer?: (
    item: any,
    model: ExpandableItem,
    node: TreeNode<ExpandableItem>,
    highlight: boolean
  ) => React.ReactNode;
    ...
    return (
      <div onKeyDown={this.handleKeyDown} onKeyPress={this.handleKeyPress}>
        ...
        <ScrollArea style={this.props.scrollPanelStyle}>
          <Tree
            onQueryCreated={this.handleRootCreated}
            {...this.props}
  ...
  navigate(dir: Dir) {
    if (this.highlighted !== undefined && this.query) {
      ...
      if (dir === Dir.UP) {
        ...
        this.highlighted = this.query.nth(this.highlighted + 1)
          ? this.highlighted + 1
          : 0;
      }
    } else if (this.query) {
...
export default class Tree extends React.Component<ITreeProps> {
  ...
  renderItems() {
    const items = this.query!.toArray(),
      ...
      maxHeight = fill
        ? 0
        : Math.min(items.length * itemHeight, this.props.height as number),
      ItemComponent = this.props.itemComponent || DefaultItemContainer;

    return (
      <div
        ref={this.containerRef}
        style={{ ...this.props.containerStyle, flex: "1 1 100%" }}
      >
            ...
            <Grid
              ...
              cellRenderer={({ rowIndex, style }) => (
                <ItemComponent
                  key={rowIndex}
                  {...{
                    style: this.getItemStyle(items[rowIndex], style),
                    node: items[rowIndex]
                  }}
                >
                  <Node
  ...
  private getItemStyle(
    ...
    return {
      ...
      ...(this.props.itemContainerStyle
        ? this.props.itemContainerStyle(node.item.data)
        : undefined)
    };
  }
  ...
  private loadNodes() {
    ...
    this.treeQuery = new TreeQuery<ExpandableItem>((item) =>
      this.props
        .childrenAccessor(item.data)
        .map((child) => this.createItem(child))
    );
    this.query = this.treeQuery
      ...
      .whereNode(
        (n) =>
          !!this.filter ||
          [...n.ancestors()].every((a) => a.item.expanded || a.isRoot)
      );
  ...
  private applyFilter(filterText?: string) {
    const regex = !filterText
      ? undefined
      : new RegExp(filterText.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "i");
    this.filter = regex;
  }
  ...
  renderExpander() {
    ...
    return (
      <Expander
        tabIndex={-1}
        href="javascript:"
        onClick={() => (model.item.expanded = !model.item.expanded)}
      >
        <i
          className={`far fa-${
            model.item.expanded ? "minus" : "plus"
          }-square fa-stack-1x`}
        />
      </Expander>
    );
...
export class Query<T> implements Iterable<TreeNode<T>> {
  ...
  private customMove: (
    current: TreeNode<T>,
    defaultNext?: TreeNode<T>
  ) => TreeNode<T> | undefined = (current, def) => def;
