...
@observer
export class DefaultOperationEditor extends DiComponent<
  IExprComponent<Operation>,
...
> {
  ...
  render() {
    const vertical = this.displayVertically(),
      ...
      operationName = this.dep.builder.viewOnly() ? (
        <OperandTextEl
          highlight={this.props.model === this.dep.highlight.expr}
          bold
            ...
            editor: (close) => (
              <Prompt>
                ...
                <PromptFooter>
                  <OperandText
                    tabbable
                    tooltip={`Remove ${this.getName()}`}
                    onClick={() => this.handleRemove(close)}
                  >
      ...
      content = (
        <>
          {!vertical ? (
            ...
            <GroupHeader color={colors.labels.teal}>
              {operationName}
            </GroupHeader>
          )}
          {!vertical ? (
  ...
  renderDefaultOperand(expr: IExpression, idx: number) {
    return (
      <React.Fragment key={idx.toString()}>
        {this.dep.builder.editorFactory.renderExpr(
          expr,
          this.createOperandElProps(idx)
        )}
      </React.Fragment>
    );
  }
  renderOperand(expr: IExpression, config: IOperationParam, idx: number) {
    const Type = config.editorType,
      ...
      exprEditor = Type ? (
        <Type model={expr} {...editProps} />
      ) : (
        this.dep.builder.editorFactory.renderExpr(expr, editProps)
      );
  ...
  renderEmptyOperand(idx: number, config?: IOperationParam) {
    const options = this.dep.builder.getExprOptions(
      this.props.model,
  ...
  getName() {
    return this.options
      ? this.options.ufName || this.options.name
  ...
  displayVertically() {
    return (
      this.props.model.operands.length > 1 &&
      this.props.model.operands.some((o) => o && o instanceof Operation)
    );
  }
}
