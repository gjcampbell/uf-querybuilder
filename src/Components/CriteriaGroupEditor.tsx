...
import { dependencies, DiComponent, IConditionExprComponent, IConfigAccessor } from './Types';
import { GroupContainer, GroupBody, ExprRow, colors, GroupHeader, OperationName, CompactMenu, OperandText, MenuSelectable } from './Style';
import { CriteriaGroup, Criterion, IExpression } from '../Models/ExpressionModels';
import { ExprConstructor } from '../Models/CodeBuilderModels';

@dependencies('builder')
@observer
export default class CriteriaGroupEditor extends DiComponent<IConditionExprComponent<CriteriaGroup>, IConfigAccessor> {
            ...
            <GroupContainer>
                ...
                <GroupBody>
                    {model.criteria.map((c, i) => (
                        <ExprRow key={i.toString()}>{this.dep.builder.editorFactory.renderExpr(c, { remove: () => this.removeChild(c) })}</ExprRow>
                    ))}
                    {this.renderAddRow()}
                </GroupBody>
            </GroupContainer>
    ...
    renderHeader() {
        const { connector, negate } = this.props.model,
            color = negate ? colors.labels.blue : connector === 'and' ? colors.labels.purple : colors.labels.teal,
            text = negate && connector === 'and' ? 'NOT ALL' : negate && connector === 'or' ? 'NOT ANY' : connector === 'and' ? 'AND' : 'OR';

        return (
            <GroupHeader color={color}>
                <OperationName ref={this.labelRef} color={color} onClick={() => (this.selectingType = true)}>
                    {text}
                </OperationName>
                <Popover
    ...
    renderAddRow() {
        return this.dep.builder.viewOnly() ? null : (
            <ExprRow>
                ...
                <OperandText tooltip="Remove Group" onClick={() => this.props.remove()} order={3}>
    ...
    private addChild(childType: ExprConstructor) {
        const child = this.dep.builder.createDefaultExpr(childType, this.props.model);
        if (child) {
    ...
    private renderOptions() {
        return [
            ...
            <MenuSelectable
                key={o.text}
                selected={this.props.model.connector === o.type && this.props.model.negate === o.negate}
