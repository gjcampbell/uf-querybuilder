...
@observer
export class CodeBuilderEditor extends React.Component<ICodeBuilderElProps> {
  private defaultScope = [];
  private highlight = observable.object({ expr: undefined }) as {
    expr?: IExpression;
  };
    ...
    return (
      ...
      >
        <>
          ...
          this.props.builder.editorFactory.canRender(this.props.builder.code)
    ...
    return this.props.builder.viewOnly() ? null : (
      <>
        {knownOptions.map((o, i) => (
          <OperandText
            key={i.toString()}
            onClick={() => this.updateCode(o.createExpr())}
          >
            <i className="fa fa-plus" /> {" Add " + (o.ufName || o.name)}
          </OperandText>
        ))}
        {allOptions.length === knownOptions.length ? null : (
          <ExprPicker
            options={otherOptions}
            text="Other..."
            onCreated={(e) => this.updateCode(e)}
          />
        )}
      </>
    );
