...
@observer
export class CriterionEditor extends DiComponent<
...
> {
  ...
  renderRemove() {
    return this.props.model.rightOperand instanceof CriteriaGroup ||
      this.dep.builder.viewOnly() ? null : (
  ...
  renderLeft() {
    ...
    return this.renderOperand(
      undefined,
      this.props.model.leftOperand,
      setter,
      false
    );
  }
  ...
  handleLeftOpChange(operand?: IExpression) {
    const prev = this.props.model.leftOperand,
      ...
      newType =
        operand && operand.getValueType ? operand.getValueType() : undefined;
  ...
  handleOperatorSelected(op: UfOperator) {
    ...
    if (this.getCompatibilitySet(prevOp) !== this.getCompatibilitySet(newOp)) {
      if (this.operatorIsHas(newOp)) {
        this.props.model.rightOperand = this.dep.builder.createDefaultExpr(
          CriteriaGroup,
          this.props.model
        )!;
      } else {
