...
import {
  ExprConstructor,
  createSortedFieldTree
} from "../Models/CodeBuilderModels";

type HasInnerMatchProps = IUnknownExprComponent<IConditionalExpression> &
  IReplaceableExpr & { parentField: NestedField };

@dependencies("builder", "fieldStore", "scope")
@observer
export class HasInnerMatch extends DiComponent<
  HasInnerMatchProps,
  IConfigAccessor & IFieldsAccessor & IScopeAccessor
> {
  ...
  render() {
    if (this.hasValidCriteria()) {
      return (
        <FillRow>
          <Provider
            fieldStore={{ fields: this.getFields(this.scope) }}
            scope={this.scope}
          >
            {this.dep.builder.editorFactory.renderExpr(this.props.model!, {
  ...
  hasValidCriteria() {
    return (
      this.props.model &&
      (this.props.model instanceof Criterion ||
        this.props.model instanceof CriteriaGroup)
    );
  }
  renderCriteriaBlank() {
    return (
  ...
  private getFields(scope: string[]) {
    const fields = this.dep.builder.getNestedFields().reduce((result, item) => {
