...
@observer
export class ChooseOperationEditor extends DiComponent<
  IExprComponent<ChooseOperation>,
  IConfigAccessor
> {
  render() {
    return (
    ...
    for (let i = 0; i < operands.length; i += 2) {
      ...
      result.push(
        <React.Fragment key={i.toString()}>
          {this.renderWhen(i, when)}
        </React.Fragment>,
        <React.Fragment key={(i + 1).toString()}>
          {this.renderThen(i + 1, then)}
        </React.Fragment>
      );
    }
      ...
      <ExprRow>
        ...
        {expr
          ? this.dep.builder.editorFactory.renderExpr(
              expr,
              this.createUpdateProps(idx)
            )
          : this.renderBlankWhen(idx)}
      </ExprRow>
    );
  }
  renderBlankWhen(idx: number) {
    return (
      <>
        ...
        <OperandText
          tooltip="Remove when-then"
          onClick={() => this.removeWhenThen(idx)}
          order={3}
        >
          <i className="fa fa-trash" />
        </OperandText>
  ...
  renderBlankThen(idx: number) {
    ...
    return this.dep.builder.viewOnly() ? (
      ...
      <ExprPicker
        color={colors.labels.indigo}
        options={options}
        onCreated={(e) => this.props.model.setOperand(idx, e)}
      />
    );
  }
  addOption() {
